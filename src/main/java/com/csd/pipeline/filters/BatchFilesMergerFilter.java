...
public class BatchFilesMergerFilter extends AbstractFilter {
    ...
    public static final InputPort<Path> IN =
        new InputPort<>("batch-dir");
    public static final OutputPort<Path> OUT =
        new OutputPort<>("mappings-file-path");
...
protected void process(Batch in, Emitter out) throws Exception {
    ...
    try (java.io.BufferedWriter writer = Files.newBufferedWriter(outputFile)) {
        ...
        while (!heap.isEmpty()) {
            ...
            // Deduplicate
            if (lastWritten == null || !lastWritten.equals(s)) {
                writer.write(s);
                writer.write(' ');
